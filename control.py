...
class ControlMixin:
    ...
    # Delay between each move step // 0.15 is ok, doesn't make mistakes yet
    move_steps_delay = 0.05
    ...
    def swipe(self, start_x, start_y, end_x, end_y):
        self.control_socket.send(self._build_touch_message(
            x=start_x, y=start_y, action=self.ACTION_DOWN))
        next_x = start_x
        next_y = start_y
        ...
        while True:
            ...
            self.control_socket.send(self._build_touch_message(
                x=next_x, y=next_y, action=self.ACTION_MOVE))

            if next_x == end_x and next_y == end_y:
                self.control_socket.send(self._build_touch_message(
                    x=next_x, y=next_y, action=self.ACTION_UP))
                break
    ...
    def tap(self, x, y):
        self.control_socket.send(self._build_touch_message(
            x=x, y=y, action=self.ACTION_DOWN))
        self.control_socket.send(self._build_touch_message(
            x=x, y=y, action=self.ACTION_UP))
